...
- `src/workflows`: first call, application follow-up, callback, nurture, and disconnect-recovery definitions.
...
- The first-call script includes identity, introduction, trust confirmation, time check, roadmap, DPA education, knowledge discovery, timeline, Realtor, lender, urgency, DTI, resources, next steps, follow-up, and closing.
...
- Existing HTTP, Twilio webhook, media WebSocket, Monday webhook, and HELUX result routes.
- Existing PostgreSQL schema and incremental migration behavior; no destructive migration is introduced.
- Existing call attempts, retry cadence, explicit callback preservation, application checkpoints, and scheduler behavior.
- Existing SMS resource library and delivery tracking.
- Existing DTI calculation, specialist handoff, live transfer, contact restriction, result completion, and application checkpoint execution.
- Existing transcript/action logging and unexpected-disconnect scheduling.
...
3. Exercise identity/wrong-number/opt-out, callback, SMS success/failure, rate question, disconnect/reconnect, transfer, and Monday outage scenarios.
...
6. Monitor tool failure codes, compliance intercept actions, callback completion, DTI delivery, application starts, latency, and disconnect recovery.
...
Automated tests cover the 20 required scenarios: normal continuation, wrong number, immediate callback, meaningful interruption, listening acknowledgement, interest-rate question, DTI request, SMS success/failure, both timeline extremes, Realtor/lender facts, changed income, callback refusal, disconnect snapshot, reconnect resume, opt-out, tool timeout, and Monday failure isolation. Characterization tests also protect the existing route/tool surface.
