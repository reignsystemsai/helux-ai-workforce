...
const { validateIntent } = require("../src/intents/intent-validator");
...
test("3 immediate callback requires and accepts customer confirmation", async () => {
  const args = { callback_at: future(), timezone: "America/New_York", reason: "Busy", sms_confirmation_consent: true, prospect_confirmed: true };
  const result = await routeIntent({ toolName: "schedule_callback", args, call, execute: async () => ({ success: true, callback_at: args.callback_at, timezone: args.timezone }) });
  assert.equal(result.success, true); assert.equal(result.customer_safe_message, "Your callback has been scheduled.");
});
...
test("15 callback refusal is rejected without creating an action", () => {
  const result = validateIntent("schedule_callback", { callback_at: future(), timezone: "America/New_York", prospect_confirmed: false, sms_confirmation_consent: false });
  assert.equal(result.valid, false); assert.equal(result.error.code, "CALLBACK_CONFIRMATION_REQUIRED");
});
